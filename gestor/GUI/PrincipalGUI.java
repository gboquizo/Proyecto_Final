...
public class PrincipalGUI extends JFrame {
	...
	public PrincipalGUI() {
		...
		itemPelicula.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				AnnadirPelicula annadirPelicula = new AnnadirPelicula();
		...
		itemVideojuego.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				AnnadirVideojuego annadirVideojuego = new AnnadirVideojuego();
				annadirVideojuego.setVisible(true);
			}
		});
		...
		itemSerie.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				AnnadirSerie annadirSerie = new AnnadirSerie();
		...
		JMenuItem itemBorrarPorTitulo = new JMenuItem("Borrar por título");
		itemBorrarPorTitulo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				 BorrarPorTitulo borrarPorTitulo = new BorrarPorTitulo();
				 borrarPorTitulo.setVisible(true);
			}
		});
		menuBorrar.add(itemBorrarPorTitulo);
		...
		itemListarPeliculas.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_M, InputEvent.CTRL_MASK | InputEvent.SHIFT_MASK));
		...
		itemListarVideojuegos.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_V, InputEvent.CTRL_MASK | InputEvent.SHIFT_MASK));
		itemListarVideojuegos.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					ListarVideojuegos ListarVideojuegos = new ListarVideojuegos();
					ListarVideojuegos.setVisible(true);
				} catch (IndexOutOfBoundsException | NoSuchElementException e1) {
		...
		mntmListarTodo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					//ListarCatalogo listarCatalogo = new ListarCatalogo();
					//listarCatalogo.setVisible(true);
		...
		JMenuItem mntmRealizarPrestamo = new JMenuItem("Realizar pr\u00E9stamo");
		mntmRealizarPrestamo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				//PrestarVideojuego PrestarVideojuego = new PrestarVideojuego();
				//PrestarVideojuego.setVisible(true);
			}
		});
		mnPrestamosdevoluciones.add(mntmRealizarPrestamo);

		JMenuItem mntmRealizarDevolucion = new JMenuItem("Realizar devoluci\u00F3n");
		mntmRealizarDevolucion.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				//DevolverVideojuego DevolverVideojuego = new DevolverVideojuego();
				//DevolverVideojuego.setVisible(true);
			}
		});
		mnPrestamosdevoluciones.add(mntmRealizarDevolucion);
		...
		mntmListarPrestados.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					//VideojuegosPrestados VideojuegosPrestados = new VideojuegosPrestados();
					//VideojuegosPrestados.setVisible(true);
				} catch (IndexOutOfBoundsException | NoSuchElementException e1) {
	...
	/**
	 * Crea un nuevo catálogo y comprueba los cambios del anterior.
	 */
	private void nuevo() {
	...
	/**
	 * Abre un nuevo catálogo y comprueba si está modificado
	 */
	private void abrir() {
		if (Gestion.isModificado()) {
			switch (JOptionPane.showConfirmDialog(filechooser, "No ha guardado, ¿Desea guardar?", "Atención",
	...
	private void guardar() {
		if (getTitle().equalsIgnoreCase("Sin título")) {
			guardarComo();
		} else {
			try {
		...
		if (JFileChooser.APPROVE_OPTION == filechooser.showDialog(filechooser, "Guardar")) {
			...
			if (filechooser.getSelectedFile().exists()) {
				...
				switch (JOptionPane.showConfirmDialog(filechooser, "El archivo ya existe, ¿Desea sobreescribir?",
						"Atención", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE)) {
